...
def swarm_group():
    """Manage AI agent swarm connecting CoPaw to remote agents."""
    pass
...
def swarm_status():
    ...
    click.echo("-" * 40)
    click.echo("Worker: Not running (use 'copaw swarm up' to start)")
...
def swarm_enqueue(capability: str, param: tuple, wait: bool):
    ...
    Examples:
        ...
        copaw swarm enqueue file:read -p path=~/Documents/file.txt -w
    """
    import sys
    import os
    # Add swarm/client to path relative to copaw package
    copaw_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    project_root = os.path.dirname(copaw_root)
...
def swarm_result(job_id: str, timeout: int):
    ...
    # src/copaw/cli/swarm_cmd.py -> project root
