...
Public API:
- ProwlrBotAgent: Main agent class
- create_model_and_formatter: Factory for creating models and formatters

Example:
    >>> from prowlrbot.agents import ProwlrBotAgent, create_model_and_formatter
    >>> agent = ProwlrBotAgent()
    >>> # Or with custom model
    >>> model, formatter = create_model_and_formatter()
"""

# ProwlrBotAgent is lazy-loaded so that importing agents.skills_manager (e.g.
# from CLI init_cmd/skills_cmd) does not pull react_agent, agentscope, tools.
# pylint: disable=undefined-all-variable
__all__ = ["ProwlrBotAgent", "create_model_and_formatter"]


def __getattr__(name: str):
    """Lazy load heavy imports."""
    if name == "ProwlrBotAgent":
        from .react_agent import ProwlrBotAgent

        return ProwlrBotAgent
