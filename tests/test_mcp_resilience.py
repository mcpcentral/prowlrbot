...
import prowlrbot.app.runner.runner as runner_module
from prowlrbot.agents.react_agent import ProwlrBotAgent
from prowlrbot.app.mcp.manager import MCPClientManager
from prowlrbot.app.runner.runner import AgentRunner
from prowlrbot.config.config import MCPClientConfig
...
async def test_register_mcp_clients_retries_once_on_closed_resource() -> None:
    ...
    agent = object.__new__(ProwlrBotAgent)
    agent.toolkit = toolkit
    agent._mcp_clients = [flaky, healthy]

    await ProwlrBotAgent.register_mcp_clients(agent)
...
async def test_reconnect_mcp_client_respects_timeout() -> None:
    ...
    ok = await ProwlrBotAgent._reconnect_mcp_client(
        _SlowClient(),
        timeout=0.01,
    )
    assert ok is False
...
async def test_query_handler_skips_session_save_when_load_not_reached(
    ...
    monkeypatch.setattr(runner_module, "ProwlrBotAgent", _FakeAgent)
