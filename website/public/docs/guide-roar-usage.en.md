...
You must send a valid ROARMessage: `from`, `to` (AgentIdentity), `intent`, and `payload`. The handler reads the task from `payload.task`, `payload.prompt`, or `payload.text`.
...
Full protocol specs live in the repo under `docs/protocols/`: ROAR-SPEC, ROAR-IDENTITY, ROAR-EXCHANGE, ROAR-CONNECT, ROAR-DISCOVERY.
